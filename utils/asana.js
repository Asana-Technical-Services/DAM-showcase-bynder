async function getCustomFieldValueByName(taskData, customFieldName) {
...
async function getCustomFieldValueByGid(taskData, customFieldGid) {
  const customFields = taskData && taskData.custom_fields;
  const result = customFields.find((field) => field.gid === customFieldGid);
  return result.display_value;
}

async function getExternalAttachmentGid(attachmentData) {
